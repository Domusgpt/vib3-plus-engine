...
- **Where tests run:** unit tests run in Node (Vitest) against the choreographer + utility modules; E2E spins up Chromium via Playwright against the static harness at `http://localhost:8000` (auto-started during `test:e2e`), and the production build step confirms Pages-ready assets. CI (`.github/workflows/ci.yml`) runs the same trio on every push/PR.
